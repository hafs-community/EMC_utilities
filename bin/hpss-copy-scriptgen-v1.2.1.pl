...
sub parse_arguments_or_abort {
  ...
  warn "verbosity $verbosity $verbose";
  $verbosity=$verbose+0 if defined($verbose) and $verbose>=0;
  $verbosity=0 if defined($quiet) && $quiet;
  warn "verbosity $verbosity $verbose";

  verbose(1, "verbosity 1 message\n");
  verbose(2, "verbosity 2 message\n");
  verbose(3, "verbosity 3 message\n");
  verbose(4, "verbosity 4 message\n");
...
sub receive_input_line {
    ...
    $name=~s(^/+)()g;
    if($name=~/\.tar\.idx$/ && $reindex) {
        verbose 3,"$name: ignore index file in source; will reindex.\n";
        return;
    }
...
if(open(LSLR,$lslr_from)) {
  ...
  while(defined($line=<LSLR>)) {
    ...
    $tgtfile=~s:^/+::g;
    if($text eq ".tar.idx") {
      $tgtfile=~s:\.tar\.idx$:\.tar:;
      $idx{$tgtfile}=$tsize;
      verbose 2,"Index of $tgtfile size $tsize\n";
    } elsif($text eq ".tar") {
      $tar{$tgtfile}=$tsize;
